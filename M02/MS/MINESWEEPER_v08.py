...
class Graphics():
	...
	class WinMode():
		...
		# def get_win_size(self,board:'Board'):
		# 	bh = board.height*self.config["b_height"]
		# 	bw = board.width*self.config["b_width"]

		# 	height =	self.config["padding"]		+ \
		# 				bh							+ \
		# 				self.config["padding"]		+ \
		# 				self.config["c_height"]		+ \
		# 				self.config["padding"]		+ \
		# 				bh							+ \
		# 				self.config["padding"]
		...
		def init_window(self,board:'Board'):
			root = tk.Tk()
			# disable window resize
			root.resizable(0,0)
			# root.geometry(self.get_win_size(board))

			# create main areas (real, central, active)
			self.frm_real = tk.Frame(
				root,padx=self.config["padding"],
				pady=self.config["padding"]
			)
			self.frm_comm = tk.Frame(
				root,padx=self.config["padding"],
				pady=self.config["padding"],
				height=self.config["c_height"],
				relief=tk.GROOVE,bd=1
			)
			self.frm_active = tk.Frame(
				root,padx=self.config["padding"],
				pady=self.config["padding"]
			)
			# attach main areas to grid
			self.frm_real.pack()
			self.frm_comm.pack(fill='x')
			self.frm_active.pack()
			...
			for i in range(board.height):
				...
				for j in range(board.width):
					...
					row.append(button)
				self.el_real.append(row)

			# create tools ribbon
			self.frm_comm.columnconfigure(0,weight=1)
			self.frm_comm.columnconfigure(1,weight=1)
			self.frm_comm.columnconfigure(2,weight=1)

			lbl_timer = tk.Label(self.frm_comm,text="000",width=10)
			...
			lbl_bombs = tk.Label(self.frm_comm,text="111",width=10)
...
def play(ms:'Game'):
	...
	with ms.graphics.renderer as gr:
		if gr.__class__ == ms.graphics.renderers[ms.graphics.KEYS]:
			...
			gr.root = gr.init_window(ms.board)
			gr.root.title = ms.title
