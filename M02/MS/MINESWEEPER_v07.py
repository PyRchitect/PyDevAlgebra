...
class Graphics():
	...
	class WinMode():	
		...
		def get_win_size(self,board:'Board'):
			...
			height =	self.config["f_padding"]	+ \
						bh							+ \
						self.config["f_padding"]	+ \
						self.config["c_height"]		+ \
						self.config["f_padding"]	+ \
						bh							+ \
						self.config["f_padding"]
			return f"{bw}x{height}"
		
		def init_window(self,board:'Board'):
			root = tk.Tk()				
			# disable window resize
			root.resizable(0,0)
			root.geometry(self.get_win_size())

			# create main areas (real, central, active)
			self.frm_real = tk.Frame(root)
			self.frm_real.rowconfigure(board.height)
			self.frm_real.columnconfigure(board.width)
			for j in board.width:
				for i in board.height:
					el_real = tk.Label(self.frm_real,text=f"{board.real[i][j]}")
					el_real.grid(row=i,column=j)
			
			self.frm_comm = tk.Frame(root)
			self.frm_active = tk.Frame(root)
