...
class Graphics():
	...
	class WinMode():
		...
		def style_config(self):
			...
			# style all possible button types
			...
			# activate hidden button type
			self.style.configure(
				f"{Board.values['hidden']}.TButton",
				relief='raised'
			)
			# activate marked button type
			self.style.configure(
				f"{Board.values['mark']}.TButton",
				relief='raised'
			)
			# real board
			self.style.configure(
		...
		def root_config(self,board:'Board',title):
			...
			self.frm_real = tk.Frame(
				self.root,padx=self.config["padding"],
			...
			lbl_timer = tk.Label(self.frm_comm,text="---",width=10)
			...
			lbl_bombs = tk.Label(self.frm_comm,text=f"{board.get_bombs_remaining():0>3}",width=10)
